...
- Tokens are never logged, never returned by `list_tasks`/`get_task_brief` or session GET list/get; reveal via Bearer `POST /api/v1/tasks/:publicId/claim` (audited). MCP `claim_task` is not implemented.
