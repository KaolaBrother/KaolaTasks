...
「终止本次交付」. Task must be `待验收` / `待修改` / `待合并` → otherwise `409` `illegal_transition`. → `已退回`; new round row `verdict: 'terminated'`; `submissions.pr_state` left as is (the PR is not closed by Kaola). `200` `{ task, round }`. Writes `状态迁移` and `评审终止` `{ task_id, round }`.

#### `GET /api/v1/stream` (#53, `registerStream` in `apps/server/src/stream.ts`)

Session SSE. No session or `待批准` → `401` `{ error: 'unauthorized' }` (same as `/api/v1/events`). `200` `text/event-stream`, `Cache-Control: no-cache`, `Connection: keep-alive`, an initial `: connected` comment, then `: ping` every 30s. Events (`event:` + `data:` JSON): `task_updated` `{ task_id, status }`; `progress` `{ task_id, percent, phase }` (`null` when omitted; never `note`); `review_round` `{ task_id, round, kind, verdict }`; `discussion_message` `{ task_id, message_id, kind, author_kind }`. Never a token, ciphertext, or `body_md`. A user may hold at most 8 concurrent streams per server instance; the ninth ends that user's oldest. `publishStreamEvent(name, payload)` (module export) is the single write seam; `buildApp` closes every open stream and clears the ping timer in `onClose`.
