...
`200` `{ task_id, status, pr_url, head_sha, round, rounds: [{ round, kind, verdict, opened_by, opened_by_task_id, opened_at, revised_at, revision_head_sha }], messages: [<msg>] (all rounds plus unrounded, by id), current: <Review Brief> }`. `pr_url` / `head_sha` are `null` before `submit_pr`.
...
「通过」. Task must be `待验收` → otherwise `409` `illegal_transition`. Sub-task whose parent is not `已完成` → `409` `{ error: 'parent_not_completed', message }`. `待验收 → 待合并`; unrounded messages (if any) are folded into a new round with `verdict: 'approved'`; otherwise a round row with `verdict: 'approved'` is still written so the verdict is recorded. `200` `{ task, round }`. Writes `状态迁移` and `评审通过` `{ task_id, round, pr_url }`. Off the response path (`scheduleWriteback` pattern) it decrypts the task credential and calls `markPullRequestReady` (then `commentOnPullRequest` with a summary if `KAOLA_REVIEW_SUMMARY_COMMENT=1`); success writes `回写` `{ task_id, transition: '翻ready', ok: true, pr_url }`, failure writes `回写` `{ ok: false, ambiguous }` and `retryPendingWritebacks` retries while the task is `待合并` / `已完成` with no successful `翻ready` row. Failure never rolls the task back.
...
**#53:** the poller and the webhook receiver select `待验收`, `待修改`, **and** `待合并` tasks. `merged` is applied only from `待合并` (→ `已完成`; a merge observed while the task is still `待验收` / `待修改` is left unchanged, since Kaola never approved it); `closed` from any of the three → `已退回`. On every poll the extended `getPullRequest` result also backfills `submissions.head_sha` / `head_branch` / `is_draft` when they are still null. When a parent task reaches `已完成`, `applyPrTerminalTransition` runs the restack step for its children (`restackChildren` in `review.ts`): each child gets a `system` discussion message plus a `review_rounds` row `kind: 'restack'`; a `待验收` child moves to `待修改`; other states only get the message. A parent reaching `已退回` writes a `system` notice on each child without changing its status.
...
- #53 review loop (`review.ts`): `评审消息` `{ task_id, message_id, kind }`; `评审开轮` `{ task_id, round, kind, verdict }`; `评审交回` `{ task_id, round, head_sha }`; `评审通过` `{ task_id, round, pr_url }`; `评审撤回` `{ task_id, round }`; `评审终止` `{ task_id, round }`; `restack` `{ task_id, parent_task_id, round, base_branch }`; `心跳` gains optional `percent` / `phase`; `状态迁移` rows for the new edges carry the usual `{ task_id, from, to }`; `回写` gains `transition: '翻ready'` rows (`{ task_id, transition, ok, pr_url? | ambiguous }`). None carries `body_md` or a token.
