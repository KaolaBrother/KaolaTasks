...
Session routes. Reviewer = any `active` user with `permission_level` `admin` or `full` (`canPublish`); anyone else → `403` `{ error: 'forbidden' }`; no session → the usual 401/302 (`sendUnauthorized`). Unknown `publicId` → `404` `{ error: 'not_found' }`. Every response is token-free (no `token`, no ciphertext); `events.details` for review events carries only short fields (`task_id`, `round`, `kind`, `verdict`, `pr_url`, `head_sha`, `message_id`), never `body_md`.
