...
stdio bridge `kaola-mcp --url <origin>` (`apps/mcp/src/main.ts`, env `KAOLA_URL`, identity `~/.kaola/device.json`) sends those headers on `POST {origin}/api/mcp`. The same package bin also exposes `kaola-mcp trust …` (not an MCP tool; DESIGN §16.7). HTTPS TLS for that POST is fail-closed extra-CA on the bridge only (add to the runtime default store, not replace; no TOFU; launcher injects extra CA only from verified `$KAOLA_HOME/trust/` state — see Env below and DESIGN §16). After `initialize` it reads the HTTP `mcp-session-id` response header (case-insensitive) and replays it on later stdin JSON-RPC lines; the session id is HTTP-only and is not added to the stdout JSON-RPC body. Without that replay, `tools/list` is `400` JSON-RPC `-32000` `Bad Request: No valid session ID provided` (Cursor live tool discovery fails). Committed example `apps/mcp/examples/mcp.json` is command + `--url` only (no `ktk_`, no forge token, no `NODE_EXTRA_CA_CERTS`).
...
Session `active`+`admin` (`requireFullAdmin` in `devices.ts` calls `canManageInstance`; else `401`/`403`). Publisher `full` is `403`. `GET …/pending` → `200` `{ devices: [{ id, hostname, fingerprint, created_at, expires_at }] }` (non-expired `status === 'pending'`, plus `active` rows that have a live `created`/`committed` pairing for missed-overlap repair). When a live pairing attempt exists, a pending row may also include `pairing_id`, `pairing_expires_at`, `requires_pairing_secret: true`, and `pairing_repair: true` for the active-repair case. It never includes the pairing secret, commitment, proof, PEM, or public key. `GET /api/v1/devices` → `{ devices: [{ id, hostname, fingerprint, status, created_at, paired_at, expires_at, last_seen, owner }] }` (`owner` is `null` while pending; else `{ kind: 'claimant', claimant_id, display_name }` or `{ kind: 'user', user_id }`). `GET /api/v1/me/devices` lists the admin's own bound rows (`devices.user_id`).

### `POST /api/v1/devices/:id/bind` (#23, extended #63)

Session `active`+`admin`. Body must contain **exactly one** of `bind_to_self` (must be `true`), `claimant_id` (positive int), `claimant_display_name` (non-empty string). Else `400` `{ error: 'invalid_body' }`. Target must be live `pending`, **or** `active` with a live `created`/`committed` pairing (missed-overlap repair: pairing secret only, owner/`expires_at` unchanged). Otherwise `409` `{ error: 'conflict', message: '电脑申请已过期或不在待授权状态。' }`. Over `max_devices` → `409` `{ error: 'conflict', message: '已达该身份的电脑台数上限。' }`. Unknown `claimant_id` → `404`. If the device has any `device_pairings` row, the body **also** requires `pairing_id` and `pairing_secret`; missing → `400 invalid_body`. Wrong secret → `403 { error: 'pairing_secret_invalid' }` (constant-time). Eight failures reject the attempt; subsequent bind is `409 pairing_secret_invalid`. Expired pairing → `409 pairing_expired`. Legacy pending devices with **no** pairing row keep the original bind body. Publishers still cannot bind. Success `200` `{ ok: true, device_id, owner }` (`owner` `{ kind: 'user', user_id }` or `{ kind: 'claimant', claimant_id, display_name }`). Writes `events.type` `电脑授权` (may include `pairing_id`, never the secret / commitment / proof / PEM). Does not claim a task and does not reveal a forge token. Bound-device `expires_at` is `paired_at + owner.device_max_age_days` (SQL/app default **90**); repair bind does not restamp it. Web **电脑** pane **绑到我自己** sends `{ bind_to_self: true }` and, when `requires_pairing_secret`, also `pairing_id` + `pairing_secret`.
...
Device proof only (`addDeviceProofHook`). Bound `active` device → `200` `{ device_id, fingerprint, hostname, status: 'active', owner }` (`owner` `{ kind: 'user', user_id }` or `{ kind: 'claimant', claimant_id, display_name }`). Active whoami also includes `instance_id` (#63). Pending signature → same `202` `authorization_required` as MCP. Never a forge token. Leftover `ktk_` Bearer → `401` + `WWW-Authenticate: Kaola-Device`. Session `GET /api/v1/me` is unchanged.

### Device pairing REST (#63, MCP-outside)

`registerPairing` in `apps/server/src/pairing.ts`. Device proof over the **raw body bytes**. Cookies are ignored and do not authorize. Leftover `ktk_` Bearer is `401` + `WWW-Authenticate: Kaola-Device`. These routes do not send `Set-Cookie`, never read Task/Claim/credential/event tables for response bodies, and never return forge tokens. `KAOLA_PAIRING_MODE` other than `private_ca` → every pairing route `404 { error: 'pairing_mode_disabled' }`. Boot `private_ca` requires a readable one-CA PEM at `KAOLA_PUBLIC_ROOT_CA_PATH` (no private-key block) that either TLS-probes `PUBLIC_URL` using **only** that root (`-CAfile` plus `-no-CApath`/`-no-CAstore`, `verify_hostname`/`verify_ip`; SNI is not identity) or verifies `KAOLA_PUBLIC_LEAF_CHAIN_PATH` to that root with complete chain checks (signature, time, `sslserver` purpose, constraints; same CA isolation) and a SAN covering the `PUBLIC_URL` DNS name or IP; `KAOLA_PAIRING_TTL_SECONDS` must be an integer in `[86400, 604800]` (default 86400). Valid unknown/pending fingerprints upsert a pending device the same way MCP does; that device still cannot list or claim.

- `POST /api/v1/device-pairings` body `{ client_nonce }` (32-byte hex). New attempt `201`, idempotent recover `200` (same pairing id / server nonce / root / `expires_at`; recover does not slide expiry). Public descriptor: `pairing_id`, `protocol_version`, `instance_id`, `origin`, `server_nonce`, `root_pem`, `root_sha256`, `expires_at`, `device_id`, `device_fingerprint`, `status`. Never the pairing secret. Invalid nonce → `400 invalid_body`. Same-device live attempt with matching `client_nonce` recovers even when the device is already `active` (approved, not yet consumed). `active` with no live attempt starts a repair pairing. `active` with a different live nonce → `409 conflict` with Chinese message to use root rotation.
- `POST /api/v1/device-pairings/:id/commit` `{ commitment }`. First commit `200 { status: 'committed', pairing_id, expires_at }`. Identical commitment is idempotent. Different commitment → `409 pairing_commitment_mismatch`.
- `POST /api/v1/device-pairings/:id/status` `{}`. `created`/`committed` have no proof. `approved` includes `approval.payload` + `approval.proof` until consumed or expiry. `consumed` does not replay the proof.
- `POST /api/v1/device-pairings/:id/complete` is **strict TLS + active device** cleanup (not a trust gate). Pending → `202 authorization_required`.
- `POST /api/v1/device-trust/next-root` strict TLS + **active** device. Overlap returns `{ root_pem, root_sha256, trust_epoch }`. No next root → `404 not_found`. Pending → `202 authorization_required`.
...
There is no REST `POST /api/v1/tasks/:publicId/submit_pr`. `submit_pr` is MCP-only (`submitPr` in `claim.ts`). `submit_revision` (#53) is MCP-only too (`submitRevision` in `review.ts`).

**#53:** `release` returns the task to `待修改` instead of `待认领` when the task already holds a `submissions` row (a revision Claim abandoned mid-way); the `状态迁移` event then carries `to: '待修改'`. Lease expiry in `sweepExpiredLeases` follows the same rule.

### Review loop REST (#53, `registerReview` in `apps/server/src/review.ts`)

Session routes. The five `POST` routes require a reviewer = any `active` user with `permission_level` `admin` or `full` (`canPublish`); anyone else → `403` `{ error: 'forbidden' }`. `GET …/review` is readable by any session except `待批准` (same population as `GET /api/v1/events`; a `待批准` session is `401`). No session → the usual 401/302 (`sendUnauthorized`). Size caps: `body_md` ≤ 20000 chars, each anchor string ≤ 2000 chars, `open_review_round` `items` ≤ 50, `report_progress` `phase` ≤ 200 chars — over the cap is `400 invalid_body`. Unknown `publicId` → `404` `{ error: 'not_found' }`. Every response is token-free (no `token`, no ciphertext); `events.details` for review events carries only short fields (`task_id`, `round`, `kind`, `verdict`, `pr_url`, `head_sha`, `head_verified`, `message_id`), never `body_md`.

Message `kind` enum (shared by REST and MCP): `blocking` | `suggestion` | `question` | `answer` | `note` | `resolution`. `anchor` is optional `{ path?: string, line?: number, head_sha?: string, url?: string }` — every key optional, only those four keys accepted, `line` a non-negative integer (stored as JSON text; the Web composer sends `url` alone when a pasted link has no parseable path). `author_kind` on the wire: `reviewer` (session user), `agent` (device Claim), `system` (restack / parent notices), `forge` (reserved for imported comments; not written in #53).

Review Brief (returned by `GET …/review` as `current` and by MCP `get_review_feedback`): `{ task_id, pr_url, round, kind, head_sha, base_branch, verdict, blocking: [<msg>], non_blocking: [<msg>], thread: [<msg>], source_trust: 'internal' }`. `<msg>` = `{ id, round, author_kind, author, kind, body_md, anchor, reply_to, resolves, resolved, created_at }` (`resolved` is `true` once a `resolution` message names it via `resolves`). Before any round is opened `round` is `0`, `kind` is `'review'`, `verdict` is `null`, and `blocking` / `non_blocking` / `thread` list the not-yet-rounded messages (`round: null`).

#### `GET /api/v1/tasks/:publicId/review`

`200` `{ task_id, status, pr_url, head_sha, forge_head_sha, forge_head_seen_at, head_stale, round, rounds: [{ round, kind, verdict, opened_by, opened_by_task_id, opened_at, revised_at, revision_head_sha }], messages: [<msg>] (all rounds plus unrounded, by id), current: <Review Brief> }`. `pr_url` / `head_sha` are `null` before `submit_pr`.

**#54:** `forge_head_sha` (string | null) and `forge_head_seen_at` (unix seconds | null) are the PR head most recently observed on the forge — written by every poller tick and by the approve live check, `null` until the first observation and reset to `null` by `submit_revision`. `head_stale` is `true` only when both `head_sha` and `forge_head_sha` are non-null and differ. The Review Brief (`current`, MCP `get_review_feedback`) is unchanged.

#### `POST /api/v1/tasks/:publicId/review/messages`

Body `{ body_md: string (non-empty), kind, anchor?, reply_to?: number, resolves?: number }`. Task must be `待验收` / `待修改` / `待合并` → otherwise `409` `{ error: 'illegal_transition', message }`. Bad body / unknown `kind` / `resolves` not a `blocking` message of this task → `400` `{ error: 'invalid_body' }`. `201` `<msg>` with `round: null` (unrounded). Writes `events` `评审消息` `{ task_id, message_id, kind }`. Never changes `tasks.status`.

#### `POST /api/v1/tasks/:publicId/review/rounds`

「提交本轮意见」. No body. Task must be `待验收` → otherwise `409` `illegal_transition`. No unrounded reviewer messages → `409` `{ error: 'no_pending_messages', message: '没有待提交的评审意见。' }`. Assigns every unrounded message to a new `review_rounds` row (`kind: 'review'`, `round = submissions.review_round + 1`, `opened_by_user_id` = the session user). If at least one of them is `blocking`: `verdict: 'changes_requested'`, `待验收 → 待修改`, `状态迁移` event. Otherwise `verdict` `null` and the task stays `待验收`. `201` `{ task, round: <round row>, current: <Review Brief> }`. Writes `events` `评审开轮` `{ task_id, round, kind, verdict }`.

#### `POST /api/v1/tasks/:publicId/review/approve`

「通过」. Task must be `待验收` → otherwise `409` `illegal_transition`. Sub-task whose parent is not `已完成` → `409` `{ error: 'parent_not_completed', message }`.

**#54 head check (before the transition):** the route decrypts the task credential and calls `createForgeAdapter(task.repoForge, { baseUrl, timeoutMs: MARK_READY_TIMEOUT_MS }).getPullRequest({ token }, submission.prUrl)` once, on the response path; a non-empty `head_sha` from it is persisted to `submissions.forge_head_sha` / `forge_head_seen_at`. Then: recorded `submissions.head_sha` `null` → it is backfilled from the forge head and the approval proceeds (`head_verified: true`); equal → proceeds (`head_verified: true`); different → `409` `{ error: 'head_sha_stale', message, recorded_head_sha, forge_head_sha }` with no state change, no round row and no `评审通过` event. If the forge cannot be read (fetch rejection, non-2xx, missing credential, or `head_sha === ''`) the stored `forge_head_sha` from the last poller tick is used instead: present and different → the same `409`; otherwise the approval proceeds fail-open with `head_verified: false`. The 409 body carries only the two shas and a Chinese `message`; never a token.

Because the live check is an `await`, the `待验收` guard above is re-checked **inside** the approval transaction: if the task is no longer `待验收` or its submission is no longer `pr_state: 'open'` by then (a poller / webhook `已退回` landed during the forge round-trip, or a second 「通过」 committed first), the transaction writes nothing and the route answers `409` `illegal_transition` — never a `500`, never an overwrite of the terminal state, and no Draft → ready flip is scheduled.

`待验收 → 待合并`; unrounded messages (if any) are folded into a new round with `verdict: 'approved'`; otherwise a round row with `verdict: 'approved'` is still written so the verdict is recorded. `200` `{ task, round, head_sha, head_verified }` (`head_sha` is the recorded head the approval is anchored to, after any backfill). Writes `状态迁移` and `评审通过` `{ task_id, round, pr_url, head_sha, head_verified }`. Off the response path (`scheduleWriteback` pattern) it decrypts the task credential and calls `markPullRequestReady` (then `commentOnPullRequest` with a summary if `KAOLA_REVIEW_SUMMARY_COMMENT=1`); success writes `回写` `{ task_id, transition: '翻ready', ok: true, pr_url }`, failure writes `回写` `{ ok: false, ambiguous }` and `retryPendingWritebacks` retries while the task is `待合并` / `已完成` with no successful `翻ready` row. Equal same-process attempts for the same task/PR/round share one in-flight promise (see "Status write-back" below). Failure never rolls the task back.

#### `POST /api/v1/tasks/:publicId/review/withdraw`

「撤回通过」. Task must be `待合并` → otherwise `409` `illegal_transition`. `待合并 → 待修改`; new round row `kind: 'review'`, `verdict: 'withdrawn'` (unrounded messages folded in). `200` `{ task, round }`. Writes `状态迁移` and `评审撤回` `{ task_id, round }`.

#### `POST /api/v1/tasks/:publicId/review/terminate`

「终止本次交付」. Task must be `待验收` / `待修改` / `待合并` → otherwise `409` `illegal_transition`. → `已退回`; new round row `verdict: 'terminated'`; `submissions.pr_state` becomes `terminated` (the PR itself is not closed by Kaola; the submission just stops being live, so a reopened task may deliver a new PR). `200` `{ task, round }`. Writes `状态迁移` and `评审终止` `{ task_id, round }`.

#### `GET /api/v1/stream` (#53, `registerStream` in `apps/server/src/stream.ts`)

Session SSE. No session or `待批准` → `401` `{ error: 'unauthorized' }` (same as `/api/v1/events`). `200` `text/event-stream`, `Cache-Control: no-cache`, `Connection: keep-alive`, an initial `: connected` comment, then `: ping` every 30s. Events (`event:` + `data:` JSON): `task_updated` `{ task_id, status }` (also published, with the unchanged status, when a poller tick observes a forge head different from the previously stored `forge_head_sha`, #54); `progress` `{ task_id, percent, phase }` (`null` when omitted; never `note`); `review_round` `{ task_id, round, kind, verdict }`; `discussion_message` `{ task_id, message_id, kind, author_kind }`. Never a token, ciphertext, or `body_md`. A user may hold at most 8 concurrent streams per server instance; the ninth ends that user's oldest. `publishStreamEvent(name, payload)` (module export) is the single write seam; `registerStream` ends that app's open streams and clears their ping timers in a `preClose` hook (an `onClose` hook would never run: Fastify's own server-close hook blocks on the open SSE responses).
...
**#53 additions (`review.ts`, registered by the same `createKaolaMcpServer`):**

| Tool | Input | Success `structuredContent` |
|------|--------|------------------------------|
| `submit_revision` | `task_id`, `claim_id`, `pr_url`, `head_sha`, `summary` | `{ task, pr_url, head_sha, round }` with `task.status` `待验收`; releases the lease. `pr_url` must canonicalize to the task's first submission's `pr_url` → else `isError` `{ error: 'pr_url_invalid' }` (`422`); `head_sha` equal to the current `submissions.head_sha` → `{ error: 'head_sha_unchanged' }` (`409`); no first submission → `{ error: 'use_submit_pr' }` (`409`); task not `进行中` → `illegal_transition`. Same `claim_id` fencing as `submit_pr`; repeating with the same Claim and `head_sha` is idempotent `200`. Writes `submission_revisions`, sets `review_rounds.revised_at` / `revision_head_sha` on the current round, updates `submissions.head_sha`, events `状态迁移` + `评审交回` `{ task_id, round, head_sha }`. |
| `get_review_feedback` | `task_id`, `round?` (integer) | Review Brief (see the REST section). No Claim needed; never a token. Unknown task → `not_found`; `round` beyond the current → `not_found`. |
| `post_discussion_message` | `task_id`, `claim_id`, `body_md`, `kind`, `reply_to?`, `resolves?`, `anchor?` | `<msg>` with `author_kind` `agent`. Requires the task's active lease held by this device+owner and matching `claim_id` → else `{ error: 'stale_claim' }` (`409`) / `forbidden` (`403`). `resolves` must name a `blocking` message of this task → else `invalid_body` (`400`); with `kind: 'resolution'` that blocking item reads `resolved: true` afterwards. Event `评审消息`. |
| `open_review_round` | `task_id` (parent), `claim_id` (caller's active Claim on a **child** task), `items: [{ kind, body_md, anchor? }]` (non-empty) | `{ task, round, current }`. The Claim's task must have `parent_task_id` = the parent → else `{ error: 'forbidden' }` (`403`); no active Claim / wrong `claim_id` → `stale_claim`. Parent `待验收` → `待修改`, round `kind: 'downstream_finding'`, `verdict: 'changes_requested'`, `opened_by_task_id` = child; parent in any other state → messages appended unrounded, `round` `null`, no transition. Event `评审开轮`. |

`report_progress` also accepts `percent?` (integer 0–100; anything else → `isError` `{ error: 'invalid_body' }` `400`) and `phase?` (string); both are recorded on the `心跳` event (`details.percent` / `details.phase`, omitted keys absent) and broadcast as an SSE `progress` event. `list_tasks` / `get_task_brief` / claim briefs carry `parent_task_id` (public id or `null`), `review_round`, and `pr_convention.draft: true`. `claim_task` accepts `待修改` (revision Claim; same envelope) and refuses a sub-task whose parent is not `待验收` / `待修改` / `待合并` / `已完成` with `{ error: 'parent_not_ready' }` (`409`). `submit_pr` on a task whose submission is still live (`pr_state` `open`) → `{ error: 'use_submit_revision' }` (`409`); a task reopened after `已退回` (its PR `closed` by the forge, or `terminated` by 「终止本次交付」) accepts a fresh `submit_pr` with a new PR. It accepts optional `head_sha` and `head_branch` (≤ 255 chars; supply `head_branch` on webhook-managed instances, where the poller never reads the PR, so sub-tasks can stack on it) and records `is_draft: true`. `submit_revision` for a released Claim with a different `head_sha` is `stale_claim` (a new head needs a new revision Claim).
...
There is still no REST `POST /api/v1/tasks/:publicId/submit_pr`.

**#53:** the poller and the webhook receiver select `待验收`, `待修改`, **and** `待合并` tasks. `merged` is applied only from `待合并` (→ `已完成`; a merge observed while the task is still `待验收` / `待修改` is left unchanged, since Kaola never approved it); `closed` from any of the three → `已退回`. On every poll the extended `getPullRequest` result also backfills `submissions.head_sha` / `head_branch` / `is_draft` when they are still null. **#54:** every poll additionally records the observed head (`head_sha !== ''`) into `submissions.forge_head_sha` / `forge_head_seen_at` (overwriting, not NULL-only), and publishes SSE `task_updated` `{ task_id, status }` when that value differs from the previously stored one; it never rewrites a non-null `submissions.head_sha` and never changes the task status because of a head mismatch — refusing the approval is the approve route's job. When a parent task reaches `已完成`, `applyPrTerminalTransition` runs the restack step for its children (`restackChildren` in `review.ts`): each child gets a `system` discussion message plus a `review_rounds` row `kind: 'restack'`; a `待验收` child moves to `待修改`; other states only get the message. A parent reaching `已退回` writes a `system` notice on each child without changing its status.
...
**Same-process in-flight coalescing.** `shareInFlightWrite` keeps a `WeakMap<AppDb, Map<key, Promise<void>>>`; `attemptWriteback` keys by task public id + transition + PR URL, while `review.ts`'s `attemptMarkReady` also includes the review round. Concurrent response-path and poller retries for an equal key await the same promise, so they produce one forge write and one outcome event. The entry is deleted in `finally`, preserving later explicit transitions and the existing failure retry rules. Scope is one process and one DB handle; this is neither a durable queue nor a cross-process exactly-once guarantee, and no token is retained in the map.
...
SQL from `createDb` (`CREATE TABLE IF NOT EXISTS users`): `id`, `provider`, `remote_id`, `username`, `display_name`, `status`, `permission_level`, `password_hash TEXT` (nullable, #28), `trusted_automation INTEGER NOT NULL DEFAULT 0` (#16), `device_max_age_days` (default **90**, #63) / `max_devices` / `device_idle_days`; UNIQUE `(provider, remote_id)`. Unique index `users_local_username` on `lower(trim(username))` WHERE `provider = 'local'`. On an existing sqlite file, `createDb` `ALTER TABLE`s missing columns (`trusted_automation`, device policy columns, `password_hash TEXT`) and swallows "duplicate column name". If `users`/`claimants`.`device_max_age_days` still has SQLite DEFAULT 30, a transactional rebuild copies rows unchanged and sets DEFAULT 90 so later inserts are not stuck at 30; stored 30 values and existing `devices.expires_at` are not rewritten. Then `promoteEarliestLoginableAdmin`: if no `active`+`admin` with provider `local`/`gitlab`/`gitea`, the earliest `active`+`full` among those providers becomes `admin`; leftover GitHub `full` is not promoted and does not count as a loginable admin.
...
**#53 columns** (added by `ALTER TABLE … ADD COLUMN`, idempotent): `head_sha TEXT` (null until `submit_pr` supplies it or the poller backfills it), `head_branch TEXT`, `is_draft INTEGER NOT NULL DEFAULT 0`, `review_round INTEGER NOT NULL DEFAULT 0`. An existing database keeps every row with those defaults.

**#54 columns** (same idempotent `ALTER TABLE … ADD COLUMN`): `forge_head_sha TEXT` and `forge_head_seen_at INTEGER`, both nullable — the PR head last observed on the forge by the poller or the approve live check, and when. `submit_revision` resets both to `NULL` when it records a new `head_sha`. Existing rows open with `NULL`.

**#53 tables:** `submission_revisions` (`id`, `submission_id`, `lease_id` UNIQUE, `round`, `head_sha`, `summary`, `submitted_at`); `review_rounds` (`id`, `task_id`, `round`, `kind` `review|downstream_finding|restack`, `opened_by_user_id`, `opened_by_task_id`, `verdict` nullable `changes_requested|approved|terminated|withdrawn`, `opened_at`, `revised_at`, `revision_head_sha`; UNIQUE `(task_id, round)`); `discussion_messages` (`id`, `task_id`, `round` nullable, `author_kind`, `author_user_id`, `author_device_id`, `kind`, `body_md`, `anchor` JSON text nullable, `reply_to_message_id`, `resolves_message_id`, `created_at`). `tasks` gains `parent_task_id INTEGER` (nullable, `tasks.id` of the parent).
...
- #53 review loop (`review.ts`): `评审消息` `{ task_id, message_id, kind }`; `评审开轮` `{ task_id, round, kind, verdict }`; `评审交回` `{ task_id, round, head_sha }`; `评审通过` `{ task_id, round, pr_url, head_sha, head_verified }` (#54); `评审撤回` `{ task_id, round }`; `评审终止` `{ task_id, round }`; `restack` `{ task_id, parent_task_id, round, base_branch }`; `心跳` gains optional `percent` / `phase`; `状态迁移` rows for the new edges carry the usual `{ task_id, from, to }`; `回写` gains `transition: '翻ready'` rows (`{ task_id, transition, ok, pr_url? | ambiguous }`). None carries `body_md` or a token.
...
Optional: `PUBLIC_URL` default `http://localhost:31415` (trailing slash stripped via `trimTrailingSlash` / `replace(/\/+$/, '')` in `auth.ts`; write-back uses the same default with `replace(/\/+$/u, '')`). When the trimmed value `startsWith('https:')`, TLS follows DESIGN §12 #46 two-mode contract: `DEBUG_PRIVATE_CA` (enrolled machines; root-signed leaf SAN = `<public-host>`; `NODE_EXTRA_CA_CERTS` on the MCP bridge only) or `STABLE_PUBLIC_CA` (clean-machine default trust; ACME DNS-01 fullchain for `<production-subdomain>`). Cookie `Secure` / `trustProxy` (below) assume the terminator already presented whatever that mode requires — they do not make a CN-only self-signed leaf trusted. Real host / `<https-port>` / DNS provider / cert identity are not in this repo. `kaola-mcp --url` (`runStdioBridge` in `apps/mcp/src/main.ts`) uses runtime-default TLS verification. Optional **client** env is `KAOLA_HOME` / `KAOLA_URL` — none of these is required at server boot, and none is a new MCP tool or HTTP route. Direct-run / package-bin launcher (`resolveLauncherTrust`) injects `NODE_EXTRA_CA_CERTS` only from verified `$KAOLA_HOME/trust/` state after `kaola-mcp trust install`; caller extra-CA is not a trust source, and public mode (no state) fail-closes if that env is set. HTTPS then adds the verified PEM to the runtime default roots via `node:https.request` (`ca: [...defaultRuntimeCAs, extraPem]`), does not replace the store, and does not set `rejectUnauthorized: false`. Unset extra CA uses `fetch` with default trust. The bridge never fetches a CA from the origin (no TOFU). `NODE_TLS_REJECT_UNAUTHORIZED=0` and `--insecure` are not part of the client contract. Operator schemes: DESIGN §16 / §16.7. On `DEBUG_PRIVATE_CA` enrolled machines only, extra CA is scoped to the user-local MCP process; it is never a `STABLE_PUBLIC_CA` / clean-machine default, and its local path/value is not committed. `KAOLA_ADMINS` if set is **ignored** (not an invite list, does not grant GitHub login, does not fail boot). Process `index.ts`: `PORT` default `'31415'`, `HOST` default `'0.0.0.0'`, `SQLITE_PATH` default `':memory:'` (compose overrides to `/data/kaola.sqlite`; non-compose unset still in-memory). Optional `WEB_DIST` and `VITE_DEV_TARGET` (not required by `registerAuth`). Optional `KAOLA_HOME` for `kaola-mcp` (default `~/.kaola`).
...
**#53:** the result is `{ state, head_sha, draft, head_branch }`. `head_sha`: GitHub/Gitea `head.sha`, GitLab `sha` (`''` when absent). `draft`: GitHub `draft === true`; GitLab `draft === true || work_in_progress === true`; Gitea `draft === true` or a title starting with `WIP:` (case-insensitive). `head_branch`: GitHub/Gitea `head.ref`, GitLab `source_branch` (`''` when absent).

### `markPullRequestReady(cred, prUrl)` (#53)

Flips a Draft PR/MR to ready. Same URL parsing and host rule as `getPullRequest`. GitHub: `GET` the PR (REST) for `node_id`, then `POST https://api.github.com/graphql` with mutation `markPullRequestReadyForReview(input: { pullRequestId })`; a GraphQL `errors` array rejects. GitLab: `GET` the MR, then `PUT /api/v4/projects/:id/merge_requests/:iid` with the title stripped of a leading `Draft:` / `WIP:` / `[Draft]` / `(Draft)` prefix (when the MR is not draft the PUT is skipped). Gitea: `GET` the PR, then `PATCH /api/v1/repos/:owner/:repo/pulls/:index` with the title stripped of a leading `WIP:` prefix (skipped when the title has none). Non-OK on any step rejects `markPullRequestReady: ${kind} responded ${status}`. Honors `timeoutMs` (#37). Covered by `mark-pull-request-ready.shared.test.ts` against all three stub backends.

### `commentOnPullRequest(cred, prUrl, body)` (#53)

Posts a comment on the PR/MR: GitHub `POST /repos/{owner}/{repo}/issues/{number}/comments`; Gitea `POST /api/v1/repos/{owner}/{repo}/issues/{number}/comments`; GitLab `POST /api/v4/projects/:id/merge_requests/:iid/notes`. Non-OK rejects `commentOnPullRequest: ${kind} responded ${status}`. Honors `timeoutMs`. Covered by `comment-on-pull-request.shared.test.ts`.
...
- `taskStatusSchema` — `z.enum(['待认领', '进行中', '待验收', '待修改', '待合并', '已完成', '已退回', '已取消'])` (`待修改`, `待合并` added in #53)
- `type TaskStatus`
...
Legal `transitionTaskStatus` edges in source (#53): 待认领 → 进行中, 已取消; 进行中 → 待认领, 待修改, 待验收; 待验收 → 待修改, 待合并, 已退回; 待修改 → 进行中, 已退回, 已取消; 待合并 → 待修改, 已完成, 已退回; 已退回 → 待认领, 已取消. (`待验收 → 已完成` is no longer legal.) `taskBriefSchema` also carries `parent_task_id` (`string | null`), `review_round` (non-negative integer), and `pr_convention.draft` (boolean).
