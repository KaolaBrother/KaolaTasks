...
- [安装与证书信任](../README.md#安装与证书信任) — #48 双模式 MCP 安装：`kaola-mcp trust` + launcher 只从已核验 state 注入额外 CA（DESIGN §16 / §16.7；系统/浏览器提权另一次）。#63 认领默认 `kaola-mcp pair --url`（§16.8 / ADR 0031）；不自动迁公开 CA。活网 UAT 前置清单见 [冒烟手册](smoke-test.md) 2026-09-09 两节，未执行项不得写成 PASS
